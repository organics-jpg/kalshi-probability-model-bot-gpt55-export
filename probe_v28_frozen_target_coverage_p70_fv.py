"""Frozen target-coverage p70 FV validator.

Research-only; no live bot changes or orders.

Physics hypothesis:
Target-coverage entries should only be sharpened when raw v28 probability is
already above the mid-confidence boundary churn zone. The current diagnostic
winner is simple: logit125 sharpening for p >= 0.70, otherwise raw.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_target_coverage_p70_fv_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_target_coverage_p70_fv_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_target_coverage_p70_fv_latest.md"

MIN_SETTLED = 30
BOOTSTRAP_RUNS = 5000
BOOTSTRAP_SEED = 28693
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "rule": "logit125 sharpen only when raw p >= 0.70; otherwise keep raw probability",
...
def build_report() -> dict[str, Any]:
    ...
    ranked = [
        ...
        score_variant(rows, "logit125_p70", p70_probability),
    ]
...
def current_read(ranked: list[dict[str, Any]], rows: list[dict[str, Any]], forward_markets: set[str]) -> list[str]:
    ...
    return [
        f"Frozen target-coverage p70 FV has {len(rows)} entries over {len(forward_markets)} future markets.",
