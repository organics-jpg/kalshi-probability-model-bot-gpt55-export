"""Strict-forward shadow monitor for v60: v55 FV with NO-side margin-gated exit.

Research-only. Entry is the v55 book-anchor FV candidate. Exit uses the v57
hold15/prob52 trigger. YES positions keep the plain v57 exit rule; NO positions
only exit when the YES-axis spot geometry is no more than +0.25 sigma above the
strike. This is an asymmetric market-structure candidate, not a symmetric
held-side physics law.
...
v42.POLICY = "v60_v55_bookanchor_hold15_prob52_noside_marginlte0p25_edge0_p65_stc0_600"
v42.REPORT_PREFIX = "v60_v55_no_side_margin_exit_shadow"
shadow.POLICY = v42.POLICY
shadow.EXIT_PROB_FLOOR = 0.52
shadow.EXIT_MIN_HOLD_SECONDS = 15.0
shadow.LOCK_PATH = OUT_DIR / "v60_v55_no_side_margin_exit_shadow_lock.json"
shadow.REGISTRY_PATH = OUT_DIR / "v60_v55_no_side_margin_exit_shadow_registry_latest.csv"
shadow.REPORT_MD = OUT_DIR / "v60_v55_no_side_margin_exit_shadow_monitor_latest.md"
shadow.REPORT_JSON = OUT_DIR / "v60_v55_no_side_margin_exit_shadow_monitor_latest.json"
...
def load_or_create_lock() -> dict[str, Any]:
    ...
    lock["lock_id"] = "v60_v55_no_side_margin_exit_shadow_v1"
    ...
    lock["exit"] = {
        ...
        "exit_rule": (
            "YES positions use hold15/prob52; NO positions exit only when p_side <= floor "
            "and YES-axis spot margin <= ceiling"
        ),
    }
    lock["purpose"] = (
        "Strict-forward shadow validation of v60 v55 FV with asymmetric NO-side "
        "YES-axis margin-gated probability exit."
    )
...
def write_report(lock: dict[str, Any], registry: pd.DataFrame, denom: dict[str, Any], new_count: int) -> None:
    ...
    fee_net = (
        float(pd.to_numeric(final.get("fee_net_cents"), errors="coerce").fillna(0.0).sum())
        if not final.empty
        else 0.0
    )
    gross = (
        float(pd.to_numeric(final.get("gross_pnl_cents"), errors="coerce").fillna(0.0).sum())
        if not final.empty
        else 0.0
    )
    fee_net_1c = (
