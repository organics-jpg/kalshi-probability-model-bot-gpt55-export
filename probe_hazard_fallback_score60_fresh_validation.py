"""Fresh validation for causal hazard-primary / score-min fallback trial.

This freezes the best causal diagnostic hazard-fallback row as a forward-only
trial. It takes the first eligible row by timestamp: hazard primary when it
appears first, otherwise a conservative min(book, RV Brownian) terminal prior.
...
LOCK_PATH = OUT_DIR / "profit_hazard_fallback_score60_fresh_lock.json"
REPORT_LATEST = OUT_DIR / "profit_hazard_fallback_score60_fresh_validation_latest.md"
JSON_LATEST = OUT_DIR / "profit_hazard_fallback_score60_fresh_validation_latest.json"
SELECTED_LATEST = OUT_DIR / "profit_hazard_fallback_score60_selected_latest.csv"
...
def load_lock() -> Dict[str, Any]:
    if not LOCK_PATH.exists():
        raise SystemExit(f"Missing hazard fallback score60 lock: {LOCK_PATH}")
...
def fallback_selected(rows: pd.DataFrame, fallback: Dict[str, Any]) -> pd.DataFrame:
    ...
    selected["selector"] = "fallback_score_min60"
...
def write_report(generated: str, lock: Dict[str, Any], all_metric: Dict[str, Any], fresh_metric: Dict[str, Any]) -> None:
    ...
    for path in [REPORT_LATEST, OUT_DIR / f"profit_hazard_fallback_score60_fresh_validation_{generated}.md"]:
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    for path in [JSON_LATEST, OUT_DIR / f"profit_hazard_fallback_score60_fresh_validation_{generated}.json"]:
        path.write_text(json.dumps(clean_json_local(payload), indent=2, sort_keys=True), encoding="utf-8")
    print("Hazard fallback score60 fresh validation complete")
