"""Frozen future validator for book-plus-5pp without cheap YES boundary pulls.

Research-only; no live bot changes or orders.

The physical hypothesis is that a broad book-plus-5pp rule may be useful, but
cheap YES rows below 45% can be boundary-pull traps: the payoff looks convex in
static FV while recross/path geometry is still unstable. This freezes the rule
before future scoring so the idea can fail honestly.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_book_plus05_no_cheap_yes_entry_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_book_plus05_no_cheap_yes_entry_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_book_plus05_no_cheap_yes_entry_latest.md"
...
def ensure_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "physics": "Broad book disagreement is retained, but cheap YES boundary-pull rows are skipped because static convexity can be dominated by recross/path instability.",
...
def policy_match(row: dict[str, Any]) -> bool:
    ...
    return base_tradeable(row) and delta is not None and delta >= 0.05 and not cheap_yes_boundary_pull
...
def build_report() -> dict[str, Any]:
    ...
    return {
        ...
        "rows": candidate_rows,
        "interpretation": [
            f"Frozen {POLICY} has {summary.get('entries')} future entries across {denominator_markets} future markets.",
