"""Frozen forward watch for a stricter v28 book-gap loss guard.

Research-only; no live bot changes or orders.

This v2 lane is a risk-control refinement of the first loss guard. The first
guard still allowed a high-p_hold value-over-hold suppression even when the
book gap was negative and the position could exit at a rich price. V2 only
suppresses that negative-gap value exit when fair drawdown is shallow enough to
support the physical "temporary mark" thesis.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_exit_book_gap_loss_guard_v2_state.json"
BOOK_GAP_STATE_JSON = OUT_DIR / "v28_frozen_exit_book_gap_suppression_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_exit_book_gap_loss_guard_v2_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_exit_book_gap_loss_guard_v2_latest.md"
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "physics": (
            "A high held probability alone is not enough to ignore a rich exit "
            "when the book already marks the held side below the executable bid. "
            "Negative-gap value exits are suppressed only when the fair-value "
            "drawdown is shallow, preserving temporary-mark recovery while "
            "avoiding deep adverse moves with generous exit liquidity."
        ),
        "source_artifact": "v28_frozen_exit_book_gap_loss_guard_latest.json",
    }
...
def build_report() -> dict[str, Any]:
    ...
    return {
        ...
        "blockers": blockers,
        "interpretation": [
            f"Frozen v2 loss guard has {summary.get('settled')} settled rows after its own freeze.",
            f"Post-freeze delta versus current v28 exits is {summary.get('delta_vs_current_cents')}c.",
            (
                "On the full diagnostic exit sample, v2 scored "
            ...
            (
                "On the comparable book-gap freeze window, v2 scored "
