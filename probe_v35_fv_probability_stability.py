"""Stability audit for the v35 FV probability surface.

This is a probability-model audit, not a scorer. It checks whether the v35
longer-horizon/softer-temperature gain over v34 is broad across chronological
blocks and physics regimes.
...
REPORT_MD = OUT_DIR / "v35_fv_probability_stability_latest.md"
REPORT_JSON = OUT_DIR / "v35_fv_probability_stability_latest.json"
SUMMARY_CSV = OUT_DIR / "v35_fv_probability_stability_summary_latest.csv"
BLOCK_CSV = OUT_DIR / "v35_fv_probability_stability_blocks_latest.csv"
BUCKET_CSV = OUT_DIR / "v35_fv_probability_stability_buckets_latest.csv"
...
BASE_MODEL = "v34_material_antipersist3"
CANDIDATE_MODEL = "v35_h150_t102_antipersist3"
...
def opportunity_rows(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in [
        ...
        "v34_material_antipersist3_d_sigma",
        "v35_h150_t102_antipersist3_d_sigma",
        "v35_h150_t102_antipersist3_anti_persistence_shift_dollars",
        "v35_h150_t102_antipersist3_anti_persistence_materiality_gate",
        "v35_h150_t102_antipersist3_anti_persistence_logit_weight",
...
def bucket_rows(rows: pd.DataFrame, dataset: str) -> pd.DataFrame:
    ...
    work["abs_v35_d_sigma"] = finite_float(work["v35_h150_t102_antipersist3_d_sigma"]).abs()
    features = [
        "seconds_to_close",
        "abs_v32_d_sigma",
        "abs_v35_d_sigma",
...
def write_report(summary: pd.DataFrame, blocks: pd.DataFrame, buckets: pd.DataFrame) -> None:
    ...
    better = high_rows.sort_values(["brier_delta", "rows"], ascending=[True, False]).head(16)
    ...
    show_models = {BASE_MODEL, CANDIDATE_MODEL, "v32_avg110_final60_exact", "v28_live_surface"}
...
def main() -> int:
    ...
    print("v35 FV probability stability complete")
