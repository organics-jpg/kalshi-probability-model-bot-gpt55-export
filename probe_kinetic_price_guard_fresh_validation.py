"""Fresh validation for a price/adverse guarded kinetic challenger.

The kinetic-guard sanity audit found a broader physical hypothesis than the
first guard: avoid expensive entries and only block extreme adverse path motion.
This freezes `adverse15<=100 AND ask<=70` as its own forward-only lock so it
can earn or fail fresh evidence without contaminating older locks.
...
SANITY_CSV = OUT_DIR / "kinetic_guard_physics_sanity_latest.csv"
KINETIC_PRICE_GUARD_LOCK_PATH = OUT_DIR / "profit_kinetic_price_guard_fresh_lock.json"
TARGET_GUARD = "adverse15<=100 AND ask<=70"
...
def next_full_15m_close(dt: datetime) -> pd.Timestamp:
    """Exclude the market already in progress when a new forward lock is made."""
    ts = pd.Timestamp(dt).tz_convert("UTC")
    minute = int(ts.minute)
    add_minutes = 15 - (minute % 15)
    if add_minutes == 0 and (ts.second > 0 or ts.microsecond > 0 or ts.nanosecond > 0):
        add_minutes = 15
    elif add_minutes == 0:
        add_minutes = 15
    return (ts.floor("min") + pd.Timedelta(minutes=add_minutes)).replace(second=0, microsecond=0, nanosecond=0)


def choose_guard() -> Dict[str, Any]:
    if not SANITY_CSV.exists():
        raise SystemExit(f"Missing kinetic guard sanity CSV: {SANITY_CSV}")
    rows = pd.read_csv(SANITY_CSV)
...
def load_or_create_lock(base: pd.DataFrame) -> Dict[str, Any]:
    if KINETIC_PRICE_GUARD_LOCK_PATH.exists():
        return json.loads(KINETIC_PRICE_GUARD_LOCK_PATH.read_text(encoding="utf-8"))
    ...
    lock = {
        ...
        "source_sanity_csv": str(SANITY_CSV),
    ...
    KINETIC_PRICE_GUARD_LOCK_PATH.write_text(
...
def write_report(path, generated: str, lock: Dict[str, Any], all_metric: Dict[str, Any], fresh_metric: Dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Lock file: `{KINETIC_PRICE_GUARD_LOCK_PATH}`",
        "",
        "## Metrics",
        "",
    ...
    for name, metric in [("all current ledger", all_metric), ("fresh after price-guard lock", fresh_metric)]:
        lines.append(
    ...
    if fresh_metric["base_markets"] == 0:
        lines.append("- Kinetic price-guard is now locked; no post-lock resolved markets are available yet.")
    else:
        lines.append(
    ...
    lines.append("- Keep this separate from other kinetic locks because the guard was discovered after earlier forward outcomes.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    md_latest = OUT_DIR / "profit_kinetic_price_guard_fresh_validation_latest.md"
    md_stamp = OUT_DIR / f"profit_kinetic_price_guard_fresh_validation_{generated}.md"
    json_latest = OUT_DIR / "profit_kinetic_price_guard_fresh_validation_latest.json"
    json_stamp = OUT_DIR / f"profit_kinetic_price_guard_fresh_validation_{generated}.json"
    selected_latest = OUT_DIR / "profit_kinetic_price_guard_selected_latest.csv"
    ...
    print("Profit kinetic price-guard fresh validation complete")
