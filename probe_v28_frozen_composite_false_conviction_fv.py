"""Frozen composite false-conviction FV validator for v28.

Research-only; no live bot changes or orders.

This freezes the broader false-conviction idea after it was diagnosed:
early boundary/recross rows can make raw v28 probability too sharp even when
the executable edge is positive. The validator only scores markets after its
own freeze timestamp, so it cannot promote the diagnostic sample that inspired
it.
"""
...
from probe_v28_false_conviction_physics_audit import (
    is_false_conviction_zone,
    shrink_to_book,
    shrink_to_50,
)
...
STATE_JSON = OUT_DIR / "v28_frozen_composite_false_conviction_fv_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_composite_false_conviction_fv_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_composite_false_conviction_fv_latest.md"

MIN_SETTLED = 30
MIN_ADJUSTED = 8
BOOTSTRAP_RUNS = 5000
BOOTSTRAP_SEED = 491233
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "variant": "composite_false_conviction_full_to_book",
        "rule": "inside composite false-conviction zone, replace raw probability with executable ask/book probability",
        "physics": "When BTC is early, near boundary, and recross-prone, apparent FV edge can be boundary churn rather than terminal information.",
    }
...
def composite_full_to_book(row: dict[str, Any]) -> float:
    raw = float(raw_probability(row))
    if is_false_conviction_zone(row):
        return clamp_prob(shrink_to_book(row, 1.0))
    return raw


def composite_half_to_book(row: dict[str, Any]) -> float:
    raw = float(raw_probability(row))
    if is_false_conviction_zone(row):
        return clamp_prob(shrink_to_book(row, 0.5))
    return raw


def composite_full_to_50(row: dict[str, Any]) -> float:
    raw = float(raw_probability(row))
    if is_false_conviction_zone(row):
        return clamp_prob(shrink_to_50(row, 1.0))
    return raw
...
def score_variant(rows: list[dict[str, Any]], name: str, fn: Callable[[dict[str, Any]], float]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        scored.append({
            ...
            "false_conviction": is_false_conviction_zone(row),
    ...
    return {
        ...
        "false_conviction_rows": sum(1 for row in scored if row["false_conviction"]),
...
def build_report() -> dict[str, Any]:
    ...
    ranked = [
        ...
        score_variant(future_rows, "composite_false_conviction_full_to_book", composite_full_to_book),
        score_variant(future_rows, "composite_false_conviction_half_to_book", composite_half_to_book),
        score_variant(future_rows, "composite_false_conviction_full_to_50", composite_full_to_50),
    ]
    ...
    return {
        ...
        "interpretation": [
            f"Frozen composite false-conviction FV has {len(future_rows)} entries over {len(future_markets)} future markets.",
...
def write_md(report: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Target variant: `{(report.get('freeze') or {}).get('variant')}`",
    ...
    for idx, row in enumerate(report.get("ranked") or [], start=1):
        ...
        lines.append(
            ...
            f"{row.get('false_conviction_rows')} | {row.get('wins')}/{row.get('losses')} | "
            f"{fmt(row.get('net_cents'))} | {fmt(row.get('brier_mean_delta'))} | {fmt(bboot.get('p95'))} | "
