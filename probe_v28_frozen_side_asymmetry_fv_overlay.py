"""Frozen validator for side-asymmetry + boundary-clock FV overlay.

Research-only; no live bot changes or orders.

Frozen rule:
    On the fixed target-coverage v28 entry surface, set FV probability to 0.50
    for boundary-clock composite rows. For remaining NO-side p60-70
    mid-boundary mid-recross rows, also set FV probability to 0.50. Leave all
    other raw v28 probabilities unchanged.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_side_asymmetry_fv_overlay_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_side_asymmetry_fv_overlay_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_side_asymmetry_fv_overlay_latest.md"
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "entry_policy": POLICY,
        "variant": VARIANT,
        "rule": "if boundary_clock_composite then p=0.50; else if side_asymmetry_no_p60_70_midboundary_midrecross then p=0.50; else p=raw",
        "physics": "Boundary-clock and mid-confidence NO hesitation are unresolved path states; FV confidence should collapse to coin-flip until the path resolves.",
        "source_artifact": "v28_side_asymmetry_fv_overlay_latest.json",
    }
...
def build_report() -> dict[str, Any]:
    ...
    if int(candidate.get("adjusted_rows") or 0) < MIN_ADJUSTED:
    ...
    return {
        ...
        "adjusted_rows": [
            compact(row)
            for row in rows
            if row.get("side_won") is not None and abs(adjusted_prob(row) - raw_prob(row)) > 1e-12
        ],
        "blockers": blockers,
...
def interpretation(candidate: dict[str, Any], blockers: list[str]) -> list[str]:
    notes = [
        ...
        f"Clock/side adjusted rows are {candidate.get('clock_adjusted_rows')}/{candidate.get('side_adjusted_rows')}.",
...
def write_md(report: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Clock/side adjusted: `{candidate.get('clock_adjusted_rows')}/{candidate.get('side_adjusted_rows')}`",
