"""Frozen repair candidate for target-coverage loss tags.

Research-only; no live bot changes or orders.

Frozen rule:
    Start from the target coverage policy. Skip rows matching the strongest
    physics-backed loss tags from the target-coverage attribution:
      - weak boundary turbulence: p < 0.60, abs_d_sigma <= 0.25,
        recross_hazard_score >= 0.75
      - paid high-price thin edge: ask >= 0.70 and raw edge < 0.05
    Restore the 75% coverage floor using clean repair rows ranked by lowest
...
STATE_JSON = OUT_DIR / "v28_frozen_target_loss_tag_repair_entry_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_target_loss_tag_repair_entry_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_target_loss_tag_repair_entry_latest.md"
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "danger_rule": "weak_boundary_turbulence OR paid_high_price_thin_edge",
        "repair_rule": "clean repair rows ranked by lowest recross hazard, missed markets first",
        "physics": "Weak near-boundary high-recross rows are unresolved path turbulence; high paid price with thin edge is negative convexity after spread/fee.",
    }
    STATE_JSON.write_text(json.dumps(state, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return state


def weak_boundary_turbulence(row: dict[str, Any]) -> bool:
    ...
    return p is not None and abs_d is not None and recross is not None and p < 0.60 and abs_d <= 0.25 and recross >= 0.75


def paid_high_price_thin_edge(row: dict[str, Any]) -> bool:
    ask = as_float(row.get("ask_prob"))
    edge = raw_edge(row)
    return ask is not None and edge is not None and ask >= 0.70 and edge < 0.05


def is_danger(row: dict[str, Any]) -> bool:
    return weak_boundary_turbulence(row) or paid_high_price_thin_edge(row)


def danger_reasons(row: dict[str, Any]) -> list[str]:
    out = []
    if weak_boundary_turbulence(row):
        out.append("weak_boundary_turbulence")
    if paid_high_price_thin_edge(row):
...
def interpretation(
    ...
    notes = [
        ...
        f"Target-loss rows removed: {len(built['danger'])}; repair rows added: {len(built['repairs'])}.",
    ]
    if blockers:
