"""Fresh validation for the kinetic touch-profit candidate.

This freezes the current best broad touch-hazard frontier row under a new
forward lock. It does not replace the older touch-hazard lock because the
frontier was refreshed after new outcomes were observed.
...
from probe_profit_touch_hazard_fresh_validation import FRONTIER_CSV, policy_from_record
...
KINETIC_TOUCH_LOCK_PATH = OUT_DIR / "profit_kinetic_touch_fresh_lock.json"
...
def choose_kinetic_candidate() -> Dict[str, Any]:
    ...
    eligible = rows[
        ...
        & rows["nondegenerate"]
        & rows["chooser"].astype(str).eq("kinetic_touch_score_15")
    ].copy()
    if eligible.empty:
        raise SystemExit("No nondegenerate all-split-positive kinetic touch candidate found")
    return eligible.iloc[0].to_dict()


def load_or_create_lock(base: pd.DataFrame) -> Dict[str, Any]:
    if KINETIC_TOUCH_LOCK_PATH.exists():
        return json.loads(KINETIC_TOUCH_LOCK_PATH.read_text(encoding="utf-8"))
    row = choose_kinetic_candidate()
    ...
    KINETIC_TOUCH_LOCK_PATH.write_text(
...
def write_report(path, generated: str, lock: Dict[str, Any], all_metric: Dict[str, Any], fresh_metric: Dict[str, Any]) -> None:
    ...
    for name, metric in [("all current ledger", all_metric), ("fresh after kinetic lock", fresh_metric)]:
        lines.append(
    ...
    if fresh_metric["base_markets"] == 0:
        lines.append("- Kinetic-touch candidate is now locked; no post-lock resolved markets are available yet.")
    else:
        lines.append(
    ...
    lines.append("- Keep this lock separate because the kinetic row was selected after the previous touch-hazard lock had new losses.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    md_latest = OUT_DIR / "profit_kinetic_touch_fresh_validation_latest.md"
    md_stamp = OUT_DIR / f"profit_kinetic_touch_fresh_validation_{generated}.md"
    json_latest = OUT_DIR / "profit_kinetic_touch_fresh_validation_latest.json"
    json_stamp = OUT_DIR / f"profit_kinetic_touch_fresh_validation_{generated}.json"
    selected_latest = OUT_DIR / "profit_kinetic_touch_selected_latest.csv"
    ...
    print("Profit kinetic-touch fresh validation complete")
