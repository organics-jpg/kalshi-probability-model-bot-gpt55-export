"""Fresh validation for the logit book/RV/hazard blend edge trial.

This freezes the strongest diagnostic row from the physics probability blend
audit as a forward-only trial. The row was discovered retrospectively, so it is
not promotion evidence; it must earn any trust through strict pre-resolution
registered live samples.
...
LOCK_PATH = OUT_DIR / "profit_logit_blend_edge10_fresh_lock.json"
REPORT_LATEST = OUT_DIR / "profit_logit_blend_edge10_fresh_validation_latest.md"
JSON_LATEST = OUT_DIR / "profit_logit_blend_edge10_fresh_validation_latest.json"
SELECTED_LATEST = OUT_DIR / "profit_logit_blend_edge10_selected_latest.csv"
...
def load_lock() -> Dict[str, Any]:
    if not LOCK_PATH.exists():
        raise SystemExit(f"Missing logit blend edge lock: {LOCK_PATH}")
...
def write_report(generated: str, lock: Dict[str, Any], all_metric: Dict[str, Any], fresh_metric: Dict[str, Any]) -> None:
    ...
    for path in [REPORT_LATEST, OUT_DIR / f"profit_logit_blend_edge10_fresh_validation_{generated}.md"]:
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    for path in [JSON_LATEST, OUT_DIR / f"profit_logit_blend_edge10_fresh_validation_{generated}.json"]:
        path.write_text(json.dumps(clean_json_local(payload), indent=2, sort_keys=True), encoding="utf-8")
    print("Logit blend edge10 fresh validation complete")
