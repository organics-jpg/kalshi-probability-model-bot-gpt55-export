"""Path-risk audit for the clean delayed-recheck rescue watch.
...
WATCH_JSON = OUT_DIR / "v28_frozen_soft_frontier_midprice_delayed_recheck_rescue_latest.json"
OUT_JSON = OUT_DIR / "v28_soft_frontier_delayed_recheck_clean_rescue_path_risk_latest.json"
OUT_MD = OUT_DIR / "v28_soft_frontier_delayed_recheck_clean_rescue_path_risk_latest.md"
...
def build_report() -> dict[str, Any]:
    ...
    for lane_name, strict_forward in [
        ...
        lanes.append({"lane": lane_name, "strict_forward": strict_forward, "summary": summarize(rows, strict_forward), "rows": rows})
    ...
    return {
        ...
        "interpretation": [
            ...
            (
                f"Diagnostic clean rescue suppressed rows with path: "
                f"{diagnostic.get('rows_with_post_recheck_path')}/{diagnostic.get('suppressed_rows')}; "
                f"worst post-recheck excursion {diagnostic.get('worst_min_after_recheck_bid_cents')}c; "
                f"adverse 10/25/50 rows {diagnostic.get('adverse_recheck_10c_rows')}/"
                f"{diagnostic.get('adverse_recheck_25c_rows')}/{diagnostic.get('adverse_recheck_50c_rows')}."
            ),
        ],
    }
