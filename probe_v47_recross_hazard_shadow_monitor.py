"""Strict-forward shadow monitor for v47 re-cross hazard FV."""
...
v42.POLICY = "v47_recross_sigma1_v3cap68_edge0_p65_stc0_600_prob54"
v42.REPORT_PREFIX = "v47_recross_hazard_shadow"
...
v42.shadow.LOCK_PATH = OUT_DIR / "v47_recross_hazard_shadow_lock.json"
v42.shadow.REGISTRY_PATH = OUT_DIR / "v47_recross_hazard_shadow_registry_latest.csv"
v42.shadow.REPORT_MD = OUT_DIR / "v47_recross_hazard_shadow_monitor_latest.md"
v42.shadow.REPORT_JSON = OUT_DIR / "v47_recross_hazard_shadow_monitor_latest.json"


def load_or_create_lock() -> dict:
    lock = ORIGINAL_LOAD_OR_CREATE_LOCK()
    lock["lock_id"] = "v47_recross_hazard_shadow_v1"
    lock["policy"] = v42.POLICY
    lock["recross_hazard"] = {
        "side_margin_sigma15_max": 1.0,
        "side_velocity_3m_min_dps": 0.50,
        "selected_p_side_cap": 0.68,
        "seconds_to_close_min": 0.0,
        "seconds_to_close_max": 600.0,
    }
    lock["purpose"] = "Strict-forward shadow validation of v47 re-cross hazard FV candidate."
...
def opportunity_table(predictions: pd.DataFrame, lock: dict) -> pd.DataFrame:
    out = v45_monitor.opportunity_table(predictions, lock).copy()
    if out.empty:
        return out
    ...
    hazard = (
        ...
        & side_margin_sigma15.le(1.0)
        & side_velocity_3m.ge(0.50)
    ).fillna(False)

    capped_p_side = np.where(hazard, np.minimum(selected_p_side, 0.68), selected_p_side)
    p_yes = pd.Series(np.where(selected_side.eq("yes"), capped_p_side, 1.0 - capped_p_side), index=out.index).clip(
        1e-6,
        1.0 - 1e-6,
    )
    out["p_yes"] = p_yes
    out["recross_hazard_active"] = hazard
