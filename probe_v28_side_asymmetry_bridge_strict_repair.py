"""Strict repair test for the side-asymmetry FV entry bridge.

Research-only; no live bot changes or orders.

Earlier bridge repair diagnostics allowed replacement rows that would fail the
same adjusted-FV edge test. This stricter version requires repair rows to also
clear adjusted_p - ask >= 0.02, so the candidate cannot repair a physics veto
with another physics veto.
"""
...
OUT_JSON = OUT_DIR / "v28_side_asymmetry_bridge_strict_repair_latest.json"
OUT_MD = OUT_DIR / "v28_side_asymmetry_bridge_strict_repair_latest.md"
...
def passes_adjusted_edge(row: dict[str, Any]) -> bool:
    edge = adjusted_edge(row)
    return edge is not None and float(edge) >= EDGE_FLOOR


def strict_clean_rows(
    all_rows: list[dict[str, Any]],
    markets: set[str],
    scorer: Callable[[dict[str, Any]], float],
) -> list[dict[str, Any]]:
    candidates = first_clean_by_market_scored(all_rows, markets, scorer, chronological=False)
    return [row for row in candidates if passes_adjusted_edge(row)]
...
def evaluate_scorer(name: str, scorer: Callable[[dict[str, Any]], float]) -> dict[str, Any]:
    ...
    skipped = [row for row in target if not passes_adjusted_edge(row)]
    ...
    chosen = strict_clean_rows(all_rows, forward_markets - target_markets, scorer)[:needed]
    chosen_markets = {str(row.get("market") or "") for row in chosen}
    if len(chosen) < needed:
        kept_markets = {str(row.get("market") or "") for row in kept}
        extras = strict_clean_rows(all_rows, all_markets - kept_markets - chosen_markets, scorer)
        for row in extras:
            if len(chosen) >= needed:
                break
    ...
    return {
        ...
        "coverage_repaired": len(chosen) >= needed,
...
def build_report() -> dict[str, Any]:
    ...
    ranked.sort(key=lambda row: (
        not bool(row.get("coverage_repaired")),
        -float((row.get("candidate_summary") or {}).get("net_cents") or -999999.0),
    ))
    return {
        "diagnostic": "side_asymmetry_bridge_strict_repair",
...
def interpretation(rows: list[dict[str, Any]]) -> list[str]:
    ...
    return [
        f"Best strict repair scorer is {best.get('scorer')} with net {cand.get('net_cents')}c and coverage {cand.get('coverage_pct')}%.",
        f"Coverage repaired: {best.get('coverage_repaired')} using {best.get('chosen_repairs')}/{best.get('needed_repairs')} repair rows.",
...
def write_md(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# v28 Side-Asymmetry Bridge Strict Repair",
        "",
    ...
    lines.extend([
        ...
        "## Ranking",
        "",
        "| rank | scorer | repaired | repairs | needed | coverage | net c | delta c | W/L | skipped net c | repair net c |",
        "|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ])
    for idx, row in enumerate(report.get("ranked") or [], start=1):
        ...
        lines.append(
            f"| {idx} | {row.get('scorer')} | {row.get('coverage_repaired')} | {row.get('chosen_repairs')} | "
            f"{row.get('needed_repairs')} | {fmt(cand.get('coverage_pct'))} | {fmt(cand.get('net_cents'))} | "
