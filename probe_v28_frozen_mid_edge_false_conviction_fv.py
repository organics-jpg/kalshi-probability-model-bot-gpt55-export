"""Frozen FV challenger for mid-edge boundary false conviction.

Research-only; no live bot changes or orders.

Physics hypothesis:
    On the target-coverage surface, the worst repeated directional-loss bucket
    is not no-edge noise. It is a medium 4-8pp raw edge while BTC is still early,
    close to the strike, and recross-prone. That is a false-conviction basin:
    the model sees enough edge to enter, but the path has not escaped the
    strike's repeated-crossing geometry. This FV challenger shrinks only that
    band toward 50, leaving stronger/farther/later rows alone.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_mid_edge_false_conviction_fv_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_mid_edge_false_conviction_fv_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_mid_edge_false_conviction_fv_latest.md"

MIN_SETTLED = 30
BOOTSTRAP_RUNS = 5000
BOOTSTRAP_SEED = 286337
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "rule": "shrink halfway toward 50 only when 4pp<=raw_edge<8pp, stc>=720, abs_d<=0.45, recross>=0.55",
        "physics": "Medium edge near an early recrossing boundary is a false-conviction basin, not durable escape from the strike.",
    }
...
def is_false_conviction(row: dict[str, Any]) -> bool:
    edge = raw_edge(row)
...
def mid_edge_false_conviction_shrink(row: dict[str, Any]) -> float:
    raw = float(raw_probability(row))
    if is_false_conviction(row):
...
def score_variant(rows: list[dict[str, Any]], name: str, fn: Callable[[dict[str, Any]], float]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        scored.append({
            ...
            "false_conviction": is_false_conviction(row),
    ...
    return {
        ...
        "false_conviction_rows": sum(1 for row in scored if row.get("false_conviction")),
...
def build_report() -> dict[str, Any]:
    ...
    ranked = [
        ...
        score_variant(future_rows, str(state.get("variant")), mid_edge_false_conviction_shrink),
    ]
    ...
    return {
        ...
        "interpretation": [
            f"Frozen mid-edge false-conviction FV has {len(future_rows)} entries over {len(future_markets)} future markets.",
