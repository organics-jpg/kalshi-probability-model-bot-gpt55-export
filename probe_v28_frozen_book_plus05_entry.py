"""Frozen future validator for the broad book-plus-5pp entry lane.

Research-only; no live bot changes or orders.

`book_plus_05` is the broad positive-gross target-coverage entry lane in the
candidate runway. Its diagnostic sample is too simulation-heavy for promotion,
so this freezes the exact rule and scores only future rows.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_book_plus05_entry_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_book_plus05_entry_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_book_plus05_entry_latest.md"
...
def ensure_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "rule": "ask 1-90c and v28_minus_ask_prob >= 0.05",
        "physics": "Only enter when raw v28 FV is at least 5 percentage points richer than the executable book ask; tests whether disagreement with the book is real edge or rejected-row illusion.",
...
def write_md(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# v28 Frozen Book Plus 05 Entry",
        "",
        "Future-only validator for the broad book-plus-5pp entry lane. No live orders.",
        "",
