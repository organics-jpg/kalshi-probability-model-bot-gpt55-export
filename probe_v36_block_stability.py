"""Block-stability audit for the v36 FV probability surface.

This is a probability-model audit, not a scorer. It compares v36 against v34
and v35 across chronological market blocks in both replay denominators.
...
REPORT_MD = OUT_DIR / "v36_block_stability_latest.md"
REPORT_JSON = OUT_DIR / "v36_block_stability_latest.json"
SUMMARY_CSV = OUT_DIR / "v36_block_stability_summary_latest.csv"
BLOCK_CSV = OUT_DIR / "v36_block_stability_blocks_latest.csv"
...
CANDIDATE = "v36_piecewise_h150_t102_antipersist3"
BASES = ["v34_material_antipersist3", "v35_h150_t102_antipersist3"]
...
def write_report(summary: pd.DataFrame, blocks: pd.DataFrame) -> None:
    ...
    for _, row in stab.iterrows():
        lines.append(
            ...
            f"{int(row['brier_improved_blocks'])}/{int(row['blocks'])} | {int(row['logloss_improved_blocks'])}/{int(row['blocks'])} | "
...
def main() -> int:
    ...
    print("v36 block stability complete")
