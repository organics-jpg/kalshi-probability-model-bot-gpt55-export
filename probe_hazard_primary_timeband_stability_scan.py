"""Causal stability scan for time-bounded hazard-primary arbitration.

The latest strict hazard loss fired early in the market. This probe tests the
physical prior that first-passage/touch hazard may need time-to-close shrinkage:
allow hazard primary only inside a bounded seconds-to-close window, otherwise
fall back to a high-coverage terminal prior. Arbitration is timestamp-causal.
...
REPORT_MD = OUT_DIR / "hazard_primary_timeband_stability_scan_latest.md"
REPORT_JSON = OUT_DIR / "hazard_primary_timeband_stability_scan_latest.json"
SUMMARY_CSV = OUT_DIR / "hazard_primary_timeband_stability_summary_latest.csv"
BLOCK_CSV = OUT_DIR / "hazard_primary_timeband_stability_blocks_latest.csv"
SLICE_CSV = OUT_DIR / "hazard_primary_timeband_stability_slices_latest.csv"
...
class Candidate:
    ...
    hazard_min_seconds: float = 60.0
    hazard_max_seconds: Optional[float] = None
    ...
    def hazard_policy(self) -> HazardPolicy:
        return HazardPolicy(
            ...
            0.0,
            80.0,
            self.hazard_min_seconds,
            "touch_loss15<=0.80",
        )

    @property
    def label(self) -> str:
        max_part = "no_max" if self.hazard_max_seconds is None else f"max{int(self.hazard_max_seconds)}"
        return f"hazard>={self.hazard_min_score:g}; sec>={self.hazard_min_seconds:g}; hazard_{max_part}_else_{self.fallback.name}"
...
HAZARD_MAX_SECONDS = [None, 900.0, 780.0, 660.0, 540.0, 420.0]
HAZARD_FLOORS = [0.45, 0.50]
CANDIDATES = [
    Candidate(
        name=f"hazard{int(floor * 100):02d}_{'nomax' if max_sec is None else 'max' + str(int(max_sec))}_else_{fallback.name}",
        hazard_min_score=floor,
        fallback=fallback,
        hazard_max_seconds=max_sec,
    )
    for floor in HAZARD_FLOORS
    for max_sec in HAZARD_MAX_SECONDS
...
def hazard_selected(base: pd.DataFrame, side_rows: pd.DataFrame, candidate: Candidate) -> pd.DataFrame:
    ...
    if candidate.hazard_max_seconds is not None and not selected.empty:
        selected = selected[
            pd.to_numeric(selected["seconds_to_close"], errors="coerce").le(candidate.hazard_max_seconds)
        ].copy()
    selected["selector"] = (
        f"hazard_primary_{candidate.hazard_min_score:g}_"
        f"{'nomax' if candidate.hazard_max_seconds is None else 'max' + str(int(candidate.hazard_max_seconds))}"
    )
    if selected.empty:
        return selected
    return (
...
def flatten_summary(dataset: str, candidate: Candidate, base: pd.DataFrame, selected: pd.DataFrame) -> Dict[str, Any]:
    ...
    row: Dict[str, Any] = {
        ...
        "hazard_max_seconds": candidate.hazard_max_seconds,
...
def block_summary(blocks: pd.DataFrame) -> pd.DataFrame:
    return threshold_scan.block_summary(blocks)
...
def write_report(generated: str, summary: pd.DataFrame, blocks: pd.DataFrame, slices: pd.DataFrame) -> None:
    ...
    if combined["robust"].any():
        ...
        lines.append("- No time-bounded hazard-primary row clears coverage, split, OOS, and block-stability gates.")
...
def main() -> int:
    ...
    (OUT_DIR / f"hazard_primary_timeband_stability_scan_{generated}.json").write_text(
        REPORT_JSON.read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    write_report(generated, summary, blocks, slices)
    (OUT_DIR / f"hazard_primary_timeband_stability_scan_{generated}.md").write_text(
        REPORT_MD.read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    print("Hazard primary time-band stability scan complete")
