"""Frozen forward challenger for mid-edge boundary deception.

Research-only; no live bot changes or orders.

Frozen rule:
    Start from the target coverage policy. Skip rows where the apparent raw
    edge is a mid-sized 4-8pp advantage while the physical state is still early
    and recross-prone:
      - 0.04 <= raw_edge < 0.08
      - seconds_to_close >= 720
      - abs_d_sigma <= 0.45
      - recross_hazard_score >= 0.55
    Restore the 75% coverage floor using clean repair rows ranked by physical
    stability: farthest from strike, then lowest recross, with a mild preference
    for 2-4pp raw-edge rows because current diagnostics show that band is less
    path-fragile than the 4-8pp band.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_mid_edge_boundary_deception_repair_entry_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_mid_edge_boundary_deception_repair_entry_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_mid_edge_boundary_deception_repair_entry_latest.md"
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "danger_rule": "4-8pp raw edge while early, close to strike, and high recross",
        "repair_rule": "clean repairs ranked by farthest abs_d, lowest recross, then 2-4pp edge preference",
        "physics": (
            "A medium model edge near the boundary can be a mirage: the model is confident enough "
            "to enter, but the spot path has not escaped the strike's recross basin."
        ),
    }
...
def is_mid_edge_boundary_deception(row: dict[str, Any]) -> bool:
    edge = raw_edge(row)
...
def repair_score(row: dict[str, Any]) -> tuple[float, float, int, str]:
    edge = raw_edge(row)
    preferred_edge_band = 0 if edge is not None and 0.02 <= edge < 0.04 else 1
    return (-abs_distance(row), recross_score(row), preferred_edge_band, str(row.get("ts_wall") or ""))
...
def build_candidate(all_rows: list[dict[str, Any]], target: list[dict[str, Any]], denominator: int) -> dict[str, Any]:
    ...
    danger = [row for row in target if is_mid_edge_boundary_deception(row)]
...
def build_report() -> dict[str, Any]:
    ...
    return {
        ...
        "interpretation": [
            ...
            f"Mid-edge boundary-deception rows removed: {len(built['danger'])}; repair rows added: {len(built['repairs'])}.",
...
def write_report(report: dict[str, Any]) -> None:
    ...
    for name, row in [("target", target), ("danger_removed", danger), ("repair_added", repair), ("candidate", candidate)]:
        lines.append(
    ...
    for row in report.get("danger_rows") or []:
        lines.append(
            ...
            f"{fmt(row.get('recross_hazard_score'))} | {row.get('side_won')} | {fmt(row.get('net_cents'))} |"
        )
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
