"""Frozen forward challenger for v28 probability-reduce exit suppression.

Research-only; no live bot changes or orders.

Physics hypothesis:
    The probability_reduce branch is meant to exit when the thesis weakens, but
    if held-side probability is still at least 75%, a temporary book drawdown is
    more likely turbulence than invalidation. In that state, suppressing the
    reduce exit should avoid clipping winners without reopening the full
    hold-to-settlement risk profile.
"""
...
OUT_JSON = OUT_DIR / "v28_frozen_exit_reduce_suppression_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_exit_reduce_suppression_latest.md"
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "physics": "Do not let transient probability_reduce turbulence close a still-strong held-side thesis.",
    }
...
def candidate_gross(row: dict[str, Any], p_hold_floor: float) -> float | None:
...
def summarize(rows: list[dict[str, Any]], p_hold_floor: float) -> dict[str, Any]:
    current_vals = []
    candidate_vals = []
    suppressed = []
    for row in rows:
        cur = current_exit(row)
        cand = candidate_gross(row, p_hold_floor)
...
def detail_rows(rows: list[dict[str, Any]], p_hold_floor: float) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        p_hold = exit_p_hold(row)
        suppressed = is_probability_reduce(row) and p_hold is not None and p_hold >= p_hold_floor
        out.append({
...
def build_report() -> dict[str, Any]:
    ...
    summary = summarize(rows, p_hold_floor)
    blockers = []
    ...
    return {
        "freeze": state,
        "summary": summary,
        "rows": detail_rows(rows, p_hold_floor),
        "candidate_live_ready": not blockers,
        "blockers": blockers,
        "interpretation": [
            f"Frozen reduce-suppression candidate has {summary.get('settled')} settled future rows.",
