...
from .fixed_terminal_oos import FixedTerminalGateConfig, HypothesisId


@dataclass(frozen=True)
class FixedTerminalLockedOOSRunPlan:
    schema_version: str
    generated_utc: str
    ...
    fixed_terminal_annualized_vol: float
    ...
    fat_tail_diagnostic_command: str
    fixed_terminal_oos_command: str
    notes: tuple[str, ...]


def build_fixed_terminal_locked_oos_plan(
    *,
    hypothesis_id: HypothesisId,
    run_id: str,
    dataset: str,
    ...
    return FixedTerminalLockedOOSRunPlan(
        ...
        fixed_terminal_annualized_vol=float(fixed_terminal_annualized_vol),
        gate_config=gates,
        ...
        fat_tail_diagnostic_command=(
            "python -m research_particle.fat_tail_particle_diagnostic "
            f"--run-root \"{artifact_root}\" "
            f"--output-dir \"{reports_dir}\" "
            "--stem fat_tail_particle_locked_oos_diagnostic "
            "--min-fill-prob 0.5 "
            "--counterfactual-fill-threshold 0.5"
        ),
        fixed_terminal_oos_command=(
            "python -m research_particle.fixed_terminal_oos "
            f"--candidates \"{candidate_path}\" "
            f"--labels \"{label_context_path}\" "
            f"--output-dir \"{reports_dir}\" "
            "--stem fixed_terminal_oos_locked "
...
def write_fixed_terminal_locked_oos_plan(
    plan: FixedTerminalLockedOOSRunPlan,
    output_dir: Path,
    stem: str,
) -> tuple[Path, Path]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Write a predeclared research-only locked OOS run plan for a fixed terminal hypothesis."
    )
    parser.add_argument(
        "--hypothesis-id",
        choices=["gaussian_vol45_terminal_v1"],
    ...
    parser.add_argument("--fixed-terminal-annualized-vol", type=float, default=0.45)
...
def main(argv: list[str] | None = None) -> int:
    ...
    dataset = args.dataset or f"particle_fixed_terminal_oos_{run_id}"
    ...
    plan = build_fixed_terminal_locked_oos_plan(
        ...
        fixed_terminal_annualized_vol=args.fixed_terminal_annualized_vol,
        gate_config=FixedTerminalGateConfig(
    ...
    json_path, md_path = write_fixed_terminal_locked_oos_plan(plan, args.output_dir, stem)
...
def _markdown(plan: FixedTerminalLockedOOSRunPlan) -> str:
    lines = [
        ...
        f"- fixed_terminal_annualized_vol: {plan.fixed_terminal_annualized_vol:g}",
        "",
        "## Gates",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "",
            plan.fat_tail_diagnostic_command,
            "",
            plan.fixed_terminal_oos_command,
            "```",
            "",
            "## Notes",
            "",
