...
from .dynamic_particle_oos import DynamicParticleGateConfig, HypothesisId


@dataclass(frozen=True)
class DynamicParticleLockedOOSRunPlan:
    schema_version: str
    generated_utc: str
    ...
    dynamic_particle_oos_command: str
    notes: tuple[str, ...]


def build_dynamic_particle_locked_oos_plan(
    *,
    hypothesis_id: HypothesisId,
    run_id: str,
    dataset: str,
...
) -> DynamicParticleLockedOOSRunPlan:
    ...
    return DynamicParticleLockedOOSRunPlan(
        ...
        dynamic_particle_oos_command=(
            "python -m research_particle.dynamic_particle_oos "
            f"--candidates \"{candidate_path}\" "
            f"--labels \"{label_context_path}\" "
            f"--output-dir \"{reports_dir}\" "
            "--stem dynamic_particle_oos_locked "
...
def write_dynamic_particle_locked_oos_plan(
    plan: DynamicParticleLockedOOSRunPlan,
    output_dir: Path,
    stem: str,
) -> tuple[Path, Path]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Write a predeclared research-only locked OOS run plan for a rolling-vol particle hypothesis."
    )
    parser.add_argument(
        "--hypothesis-id",
        choices=["rolling_vol_300s_v1", "rolling_vol_600s_v1"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    dataset = args.dataset or f"particle_dynamic_oos_{run_id}"
    ...
    plan = build_dynamic_particle_locked_oos_plan(
    ...
    json_path, md_path = write_dynamic_particle_locked_oos_plan(plan, args.output_dir, stem)
...
def _markdown(plan: DynamicParticleLockedOOSRunPlan) -> str:
    lines = [
    ...
    lines.extend(
        [
            ...
            "",
            plan.dynamic_particle_oos_command,
            "```",
            "",
            "## Notes",
            "",
