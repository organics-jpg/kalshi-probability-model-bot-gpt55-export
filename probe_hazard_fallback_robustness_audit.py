"""Robustness audit for hazard-primary fallback candidates.

The hazard-fallback frontier is a diagnostic scan. This audit replays each
hazard-primary fallback selector on current and v21 ledgers with split,
chronological block, and selector-source slice checks.
...
from probe_cross_dataset_profit_frontier import fmt_cents, fmt_roi, metrics_for
from probe_hazard_fallback_frontier import FALLBACKS, combined_selected
...
REPORT_MD = OUT_DIR / "hazard_fallback_robustness_audit_latest.md"
REPORT_JSON = OUT_DIR / "hazard_fallback_robustness_audit_latest.json"
SUMMARY_CSV = OUT_DIR / "hazard_fallback_robustness_summary_latest.csv"
BLOCK_CSV = OUT_DIR / "hazard_fallback_robustness_blocks_latest.csv"
SLICE_CSV = OUT_DIR / "hazard_fallback_robustness_slices_latest.csv"
HISTORY_CSV = OUT_DIR / "hazard_fallback_robustness_history_latest.csv"
...
def flatten_summary(dataset: str, name: str, label: str, base: pd.DataFrame, selected: pd.DataFrame) -> Dict[str, Any]:
    metrics = metrics_for(base, selected)
    row: Dict[str, Any] = {"dataset": dataset, "candidate": name, "label": label}
...
def history_rows() -> List[Dict[str, Any]]:
    names = {spec.name: spec.label for spec in FALLBACKS}
    rows: List[Dict[str, Any]] = []
    for path in sorted(OUT_DIR.glob("hazard_fallback_frontier_*.csv")):
        try:
...
def write_report(generated: str, summary: pd.DataFrame, blocks: pd.DataFrame, slices: pd.DataFrame, history: pd.DataFrame) -> None:
    ...
    if history.empty:
        ...
        for _, row in history[history["candidate"].eq("logit_thresh55_edge15")].tail(8).iterrows():
            lines.append(
    ...
    lines.append("- Robust offline diagnostics still require strict live pre-registration before promotion.")
...
def main() -> int:
    ...
    for dataset, loader in [("current", load_side_rows), ("v21", load_v21_side_rows)]:
        ...
        for spec in FALLBACKS:
            selected = combined_selected(base, side_rows, spec)
            selected.to_csv(OUT_DIR / f"hazard_fallback_{spec.name}_{dataset}_selected_latest.csv", index=False)
            summary_rows.append(flatten_summary(dataset, spec.name, spec.label, base, selected))
            block_out.extend(block_rows(dataset, spec.name, base, selected))
            slice_out.extend(slice_rows(dataset, spec.name, selected))
    ...
    (OUT_DIR / f"hazard_fallback_robustness_audit_{generated}.json").write_text(
        REPORT_JSON.read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    write_report(generated, summary, blocks, slices, history)
    (OUT_DIR / f"hazard_fallback_robustness_audit_{generated}.md").write_text(
        REPORT_MD.read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    print("Hazard fallback robustness audit complete")
    print(f"candidates={len(FALLBACKS)} summary_rows={len(summary)}")
