"""Frozen forward validator for conservative target-coverage FV sharpening.

Research-only; no live bot changes or orders.

Freezes the current physics-backed challenger:
logit125_p60_calm_mid_or_p75 on top of the frozen target-coverage entry
surface. It sharpens:
- p >= 0.75 always;
- 0.60 <= p < 0.75 only when recross is calm and ask <= 70c.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_target_coverage_conservative_fv_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_target_coverage_conservative_fv_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_target_coverage_conservative_fv_latest.md"

MIN_SETTLED = 30
BOOTSTRAP_RUNS = 5000
BOOTSTRAP_SEED = 28671
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "rule": "logit125 sharpen p>=0.75, or p>=0.60 only when recross<=0.60 and ask<=0.70",
        "source_artifact": "v28_target_coverage_conservative_fv_variants_latest.json",
    }
...
def build_report() -> dict[str, Any]:
    ...
    freeze_dt = parse_ts(state["freeze_ts_utc"])
    ...
    ranked = [
        ...
        score_variant(rows, "logit125_p60_calm_mid_or_p75", logit125_p60_skip_mid_loss_zone),
    ]
    ...
    return {
        ...
        "interpretation": current_read(ranked, rows, forward_markets),
    }


def current_read(ranked: list[dict[str, Any]], rows: list[dict[str, Any]], forward_markets: set[str]) -> list[str]:
    best = ranked[0] if ranked else {}
    return [
        f"Frozen conservative target-coverage FV has {len(rows)} entries over {len(forward_markets)} future markets.",
