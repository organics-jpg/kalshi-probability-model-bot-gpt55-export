"""Signed-momentum exhaustion guard audit for book/score priors.

Strict forward misses show a recurring failure mode: the selected side has
already had a large favorable 15-30 minute move, so the book/score prior can be
paying for stale momentum near the end of the interval. This probe tests a
small, fixed set of side-aligned signed-move caps on the high-coverage
book/score families.
...
REPORT_MD = OUT_DIR / "signed_momentum_exhaustion_guard_audit_latest.md"
REPORT_JSON = OUT_DIR / "signed_momentum_exhaustion_guard_audit_latest.json"
SUMMARY_CSV = OUT_DIR / "signed_momentum_exhaustion_guard_summary_latest.csv"
BLOCKS_CSV = OUT_DIR / "signed_momentum_exhaustion_guard_blocks_latest.csv"
SLICES_CSV = OUT_DIR / "signed_momentum_exhaustion_guard_slices_latest.csv"
...
BASE_SPECS = [
    ...
    ("book_margin_edge_m5", "book_p_side", 0.60, -5.0, 95.0, 120.0, True),
...
FILTER_SETS: List[Tuple[Filter, ...]] = [
    (),
    (("signed_move_30m", "<=", 75.0),),
    (("signed_move_30m", "<=", 100.0),),
    (("signed_move_30m", "<=", 125.0),),
    (("signed_move_30m", "<=", 150.0),),
    (("signed_move_30m", "<=", 200.0),),
    (("signed_move_30m", "<=", 250.0),),
    (("signed_move_15m", "<=", 75.0),),
    (("signed_move_15m", "<=", 100.0),),
    (("signed_move_15m", "<=", 125.0),),
    (("signed_move_15m", "<=", 150.0),),
    (("signed_move_15m", "<=", 200.0),),
    (("signed_move_max_15_30m", "<=", 100.0),),
    (("signed_move_max_15_30m", "<=", 150.0),),
    (("signed_move_max_15_30m", "<=", 200.0),),
    (("long_minus_short_move_30_5m", "<=", 75.0),),
    (("long_minus_short_move_30_5m", "<=", 100.0),),
    (("long_minus_short_move_30_5m", "<=", 150.0),),
]
...
def add_momentum_features(side_rows: pd.DataFrame) -> pd.DataFrame:
    out = side_rows.copy()
    for col in ["signed_move_5m", "signed_move_15m", "signed_move_30m"]:
        if col not in out.columns:
            out[col] = pd.NA
        out[col] = pd.to_numeric(out[col], errors="coerce")
    out["signed_move_max_15_30m"] = out[["signed_move_15m", "signed_move_30m"]].max(axis=1)
    out["long_minus_short_move_30_5m"] = out["signed_move_30m"] - out["signed_move_5m"]
    return out
...
def selected_for_candidate(side_rows: pd.DataFrame, base: pd.DataFrame, candidate: Candidate) -> pd.DataFrame:
    ...
    for feature, op, threshold in candidate.filters:
        if feature not in chosen.columns:
            mask &= False
            continue
        values = pd.to_numeric(chosen[feature], errors="coerce")
        if op == "<=":
...
def flatten_summary(dataset: str, candidate: Candidate, base: pd.DataFrame, selected: pd.DataFrame) -> Dict[str, Any]:
    ...
    row["all_median_signed_move_30m"] = (
        float(pd.to_numeric(selected.get("signed_move_30m"), errors="coerce").median())
        if not selected.empty
        else None
    )
    return row
...
def write_report(
    ...
    top_candidates = set(combined.head(30)["candidate"].astype(str)) if not combined.empty else set()
    for _, row in summary.sort_values(["candidate", "dataset"]).iterrows():
        if str(row["candidate"]) not in top_candidates:
            continue
        lines.append(
            ...
            f"{fmt_cents(row.get('all_median_signed_move_30m'))} | "
    ...
    lines += ["", "## Read", ""]
    if robust_count:
        lines.append("- At least one signed-momentum guard row clears diagnostic robustness; it still needs strict forward registration.")
    else:
        lines.append("- No signed-momentum guard row clears the full robustness gate.")

    REPORT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
    (OUT_DIR / f"signed_momentum_exhaustion_guard_audit_{generated}.md").write_text(
        "\n".join(lines) + "\n",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    datasets = {
        "current": add_momentum_features(load_side_rows()),
        "v21": add_momentum_features(load_v21_side_rows()),
    }
    ...
    (OUT_DIR / f"signed_momentum_exhaustion_guard_audit_{generated}.json").write_text(
        json.dumps(clean_json_local(payload), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    write_report(generated, combined, summary, blocks, slices)
    print("Signed momentum exhaustion guard audit complete")
