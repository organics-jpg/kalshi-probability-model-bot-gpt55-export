"""Forward validation for a locked book/V2 UTC-session switch.

The regime-switch scan found that a simple UTC-hour rule may explain some of
the recent book-margin failures without sacrificing recurring-market coverage.
This validator keeps the rule causal: use the book-margin anchor unless the
anchor first appears during UTC hour 04, in which case use the locked Brownian
frontier_v2 signal when it exists.
...
from probe_profit_lock_pending_signal_monitor import BOOK_HOUR04_V2_SWITCH_LOCK_PATH, select_session_switch_rows
...
LOCK_NAME = "book_hour04_v2_switch"
REPORT_MD = OUT_DIR / "profit_book_hour04_v2_switch_validation_latest.md"
REPORT_JSON = OUT_DIR / "profit_book_hour04_v2_switch_validation_latest.json"
SELECTED_CSV = OUT_DIR / "profit_book_hour04_v2_switch_selected_latest.csv"
...
def write_report(
    ...
    lines = [
        ...
        f"- Lock file: `{BOOK_HOUR04_V2_SWITCH_LOCK_PATH}`",
        "",
        "## Metrics",
        "",
    ...
    if strict_metric["base_markets"] == 0:
        lines.append("- Session-switch lock is waiting for post-boundary resolved markets.")
    ...
    lines.append("- The physical hypothesis is session-dependent microstructure, not a universal replacement for book pressure.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> int:
    generated = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%SZ")
    lock = json.loads(BOOK_HOUR04_V2_SWITCH_LOCK_PATH.read_text(encoding="utf-8"))
    ...
    selected.to_csv(OUT_DIR / f"profit_book_hour04_v2_switch_selected_{generated}.csv", index=False)

    md_stamp = OUT_DIR / f"profit_book_hour04_v2_switch_validation_{generated}.md"
    json_stamp = OUT_DIR / f"profit_book_hour04_v2_switch_validation_{generated}.json"
    ...
    print("Book hour04 V2 switch forward validation complete")
    print(
