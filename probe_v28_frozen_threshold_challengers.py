"""Second-wave frozen forward candidates for threshold sensitivity.

The first frozen clean-forward miss showed p60 candidates skipping a market
where raw v28 had p ~= 0.581 and large edge. These p58 challengers are frozen
after that observation, so they cannot get credit for the miss that inspired
them.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_threshold_challengers_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_threshold_challengers_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_threshold_challengers_latest.md"

CHALLENGERS = [
    {
        "policy": "first_side_raw_later_book_p58_edge0",
        "role": "p58_calibrated_broad_challenger",
        "physics": "Same memory rule as primary p60, but lowers the probability floor to capture high-edge 58-60% raw geometry rows.",
    },
    {
        "policy": "rmt_repetition_forget_p58_edge0",
        "role": "p58_rmt_forgetting_challenger",
        "physics": "Same RMT/repetition forgetting rule as p60, but lowers the probability floor to p58.",
    },
]
