...
$launcher = Join-Path $scriptDir 'run_v28_feature_gate_live_size1.ps1'
$storageTag = 'live_mushroom_v28_feature_gate_ask65_size1'
$expectedStrategyTag = 'mushroom_v28_feature_gate_raw05_recross60_abs085_ask65_size1_live'
...
function Start-LiveBot {
    ...
    $argumentLine = '-NoProfile -ExecutionPolicy Bypass -File "{0}" -SourceWorkspace "{1}"' -f $launcher, $resolvedSource
    Write-MonitorLog "START requested | launcher=$launcher source_workspace=$resolvedSource"
    Start-Process -FilePath 'powershell.exe' -ArgumentList $argumentLine -WorkingDirectory $repoDir -WindowStyle Hidden
}
