...
$launcher = Join-Path $scriptDir 'run_probability_lab_bot_live_size2.ps1'
$storageTag = 'live_mushroom_v28_size2'
$expectedStrategyTag = 'mushroom_v28_live_gate_ev_exit_size2'
...
function Start-LiveBot {
    ...
    Start-Process -FilePath 'powershell.exe' -ArgumentList $args -WorkingDirectory $repoDir -WindowStyle Hidden
}
...
# Codex automation sandboxes may deny process inspection. In that case, a fresh
# bot heartbeat is the best safe liveness signal available from the workspace.
