"""Fresh validation for a touch-hazard blocker-overlay challenger.

This freezes the best coverage-preserving touch-hazard overlay discovered by
the attribution scan. Because the overlay was selected after seeing existing
fresh rows, the lock starts only after the current resolved ledger. Future runs
evaluate post-lock markets without changing the original touch-hazard lock.
...
from probe_touch_hazard_blocker_overlays import Overlay, overlay_mask, select_with_overlay
from probe_profit_lock_time_boundary import effective_lock_dt


OVERLAY_CSV = OUT_DIR / "touch_hazard_blocker_overlays_latest.csv"
TOUCH_OVERLAY_LOCK_PATH = OUT_DIR / "profit_touch_hazard_overlay_fresh_lock.json"
...
def choose_overlay() -> Dict[str, Any]:
    if not OVERLAY_CSV.exists():
        raise SystemExit(f"Missing touch overlay CSV: {OVERLAY_CSV}")
    ...
    if eligible.empty:
        raise SystemExit("No all-split-positive coverage-preserving touch overlay found")
...
def load_or_create_lock(base: pd.DataFrame) -> Dict[str, Any]:
    if TOUCH_OVERLAY_LOCK_PATH.exists():
        return json.loads(TOUCH_OVERLAY_LOCK_PATH.read_text(encoding="utf-8"))
    if not TOUCH_LOCK_PATH.exists():
        raise SystemExit(f"Missing touch lock: {TOUCH_LOCK_PATH}")
    base_lock = json.loads(TOUCH_LOCK_PATH.read_text(encoding="utf-8"))
    row = choose_overlay()
    ...
    lock = {
        ...
        "base_touch_lock": str(TOUCH_LOCK_PATH),
    ...
    TOUCH_OVERLAY_LOCK_PATH.write_text(json.dumps(clean_json_local(lock), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return lock
...
def write_report(path: Path, generated: str, lock: Dict[str, Any], all_metric: Dict[str, Any], fresh_metric: Dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Lock file: `{TOUCH_OVERLAY_LOCK_PATH}`",
        "",
        "## Metrics",
        "",
    ...
    for name, metric in [("all current ledger", all_metric), ("fresh after overlay lock", fresh_metric)]:
        lines.append(
    ...
    if fresh_metric["base_markets"] == 0:
        lines.append("- Overlay challenger is now locked; no post-lock resolved markets are available yet.")
    else:
        lines.append(
    ...
    lines.append("- Keep this separate from the base touch-hazard lock because the overlay was selected after seeing earlier fresh rows.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    md_latest = OUT_DIR / "profit_touch_hazard_overlay_fresh_validation_latest.md"
    md_stamp = OUT_DIR / f"profit_touch_hazard_overlay_fresh_validation_{generated}.md"
    json_latest = OUT_DIR / "profit_touch_hazard_overlay_fresh_validation_latest.json"
    json_stamp = OUT_DIR / f"profit_touch_hazard_overlay_fresh_validation_{generated}.json"
    selected_latest = OUT_DIR / "profit_touch_hazard_overlay_selected_latest.csv"
    ...
    print("Profit touch-hazard overlay fresh validation complete")
