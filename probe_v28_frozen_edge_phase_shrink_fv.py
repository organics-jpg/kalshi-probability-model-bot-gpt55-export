"""Frozen edge-phase boundary/recross shrink FV challenger.

Research-only; no live bot changes or orders.

Physics hypothesis:
    Boundary recross is not always noise. A wide raw edge can indicate real
    directional acceleration even near the strike, while thin-edge boundary
    states are more likely to be noisy repeated-crossing regimes. This frozen
    challenger shrinks only shallow high-recross rows without a wide edge, plus
    very thin deeper pressure touches.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_edge_phase_shrink_fv_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_edge_phase_shrink_fv_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_edge_phase_shrink_fv_latest.md"

MIN_SETTLED = 30
BOOTSTRAP_RUNS = 5000
BOOTSTRAP_SEED = 286219
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "rule": "shrink halfway toward 50 when abs_d<0.45 and recross>=0.55 and edge<8pp, or when abs_d>=0.45 and recross>=0.40 and edge<2pp",
        "physics": "Boundary turbulence should forget confidence only when the executable edge is not wide; wide-edge boundary motion may be valid acceleration.",
    }
...
def build_report() -> dict[str, Any]:
    ...
    ranked = [
        ...
        score_variant(future_rows, str(state.get("variant")), edge_phase_shrink),
    ]
    ...
    return {
        ...
        "interpretation": [
            f"Frozen edge-phase shrink has {len(future_rows)} entries over {len(future_markets)} future markets.",
