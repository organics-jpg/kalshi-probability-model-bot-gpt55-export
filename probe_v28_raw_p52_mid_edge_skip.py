"""Raw p52 mid-edge false-conviction skip diagnostic.

Research-only; no live bot changes or orders.

Physics hypothesis:
    For a noisy 15-minute BTC boundary contract, an apparent 5-10pp edge can be
    a false-conviction band: large enough to trigger broad entry, but not large
    enough to dominate boundary recross, stale book, and taker spread noise.
    Thin edges may often be book-consensus rows, while very large edges are
    special convex/dislocation rows. The middle can be the trap.
"""
...
OUT_JSON = OUT_DIR / "v28_raw_p52_mid_edge_skip_latest.json"
OUT_MD = OUT_DIR / "v28_raw_p52_mid_edge_skip_latest.md"
...
def edge_prob(row: dict[str, Any]) -> float | None:
...
def should_skip(row: dict[str, Any]) -> bool:
    edge = edge_prob(row)
    return edge is not None and 0.05 <= edge < 0.10
...
def summarize(rows: list[dict[str, Any]], watched: int) -> dict[str, Any]:
    ...
    return {
        ...
        "avg_edge": avg(edge_prob(row) for row in settled_rows),
        "win_rate": (
...
def build_report() -> dict[str, Any]:
    ...
    return {
        ...
        "physics": "Middle-edge rows can be false conviction: they pass entry but do not dominate boundary/path noise.",
