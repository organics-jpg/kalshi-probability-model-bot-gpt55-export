"""Runway and miss audit for frozen empirical-Bayes p70 FV.

Research-only; no live bot changes or orders.

This mirrors the hard-p70 runway audit for the frozen empirical-Bayes p70
candidate. The goal is to distinguish "no forward evidence yet" from "the
target entry surface is seeing markets but correctly abstaining."
"""
...
from probe_v28_frozen_target_coverage_p70_empirical_bayes import STATE_JSON
...
OUT_JSON = OUT_DIR / "v28_frozen_target_coverage_p70_empirical_bayes_runway_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_target_coverage_p70_empirical_bayes_runway_latest.md"
...
def raw_bucket(p_raw: float | None) -> str:
    ...
    return "raw_ge_70_eb_adjustable"
...
def base_opportunity_summary(markets: list[dict[str, Any]]) -> dict[str, Any]:
    summary = {
        ...
        "raw_ge_70_eb_adjustable": 0,
        "missing_raw": 0,
        "near_edge_miss_lt_2pp": 0,
        "high_recross_miss_ge_75": 0,
        "eb_adjustable_unselected": 0,
    }
    for market in markets:
        ...
        for row in market.get("base_rows") or []:
            ...
            if bucket == "raw_ge_70_eb_adjustable" and str(row.get("market") or "") not in selected_markets:
                summary["eb_adjustable_unselected"] += 1
...
def build_report() -> dict[str, Any]:
    ...
    if not freeze_ts:
        return {
            ...
            "interpretation": ["Frozen empirical-Bayes p70 state does not exist yet."],
        }
    ...
    return {
        ...
        "base_opportunity_summary": summary,
        "markets": markets,
        "interpretation": interpretation(markets, summary),
    }


def interpretation(markets: list[dict[str, Any]], summary: dict[str, Any]) -> list[str]:
    if not markets:
        return ["No clean post-freeze markets exist yet for frozen empirical-Bayes p70."]
    ...
    return [
        f"Frozen empirical-Bayes p70 has {selected} selected markets, {base_unselected} markets with base rows that failed the target policy, and {no_base} markets with no target base row.",
        f"Base rows by raw-probability bucket: <60={summary.get('raw_lt_60')}, 60-70 boundary={summary.get('raw_60_70_boundary')}, >=70 EB-adjustable={summary.get('raw_ge_70_eb_adjustable')}.",
        f"Unselected EB-adjustable rows: {summary.get('eb_adjustable_unselected')}; if this stays 0, the blocker is opportunity, not EB probability scoring.",
    ]
