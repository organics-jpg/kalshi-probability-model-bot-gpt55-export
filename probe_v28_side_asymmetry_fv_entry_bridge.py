"""Entry bridge for the side-asymmetry FV overlay.

Research-only; no live bot changes or orders.

The side-asymmetry FV overlay improves probability calibration on the target
coverage surface, but calibration alone does not create PnL. This probe asks
whether using the adjusted FV directly as the entry edge would skip bad rows
while preserving the 75% coverage floor via clean repair opportunities.
"""
...
OUT_JSON = OUT_DIR / "v28_side_asymmetry_fv_entry_bridge_latest.json"
OUT_MD = OUT_DIR / "v28_side_asymmetry_fv_entry_bridge_latest.md"
...
def adjusted_edge(row: dict[str, Any]) -> float | None:
    ...
    if ask_prob is None:
        return None
    return adjusted_p(row) - ask_prob
...
def build_report() -> dict[str, Any]:
    ...
    return {
        "diagnostic": "side_asymmetry_fv_entry_bridge",
        "coverage_floor": COVERAGE_FLOOR,
        "physics": "Use clock-plus-side-asymmetry adjusted FV as the entry surface; skip rows where adjusted FV no longer clears ask, then repair coverage with clean low-recross rows.",
        "ranked": ranked,
...
def write_md(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# v28 Side-Asymmetry FV Entry Bridge",
        "",
