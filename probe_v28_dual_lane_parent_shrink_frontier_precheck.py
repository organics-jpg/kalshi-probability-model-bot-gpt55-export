"""Forced strict-replay precheck for the dual-lane parent-shrink frontier.

Research-only; no live bot changes and no orders.

This writes a separate diagnostic artifact so force replay does not overwrite
the normal frontier watch. It is not promotion evidence before the frontier's
own 30-settled-row gate.
"""
...
import probe_v28_dual_lane_parent_shrink_frontier_watch as frontier_watch


ROOT = Path(__file__).resolve().parent
OUT_DIR = ROOT / "logs" / "edge_research"
OUT_JSON = OUT_DIR / "v28_dual_lane_parent_shrink_frontier_precheck_latest.json"
OUT_MD = OUT_DIR / "v28_dual_lane_parent_shrink_frontier_precheck_latest.md"
...
def compact_union(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "frontier_label": row.get("frontier_label"),
        "frontier_weight": row.get("frontier_weight"),
...
def build_report() -> dict[str, Any]:
    old_force = os.environ.get("V28_DUAL_PARENT_SHRINK_FRONTIER_FORCE_REPLAY")
    os.environ["V28_DUAL_PARENT_SHRINK_FRONTIER_FORCE_REPLAY"] = "1"
    try:
        report = frontier_watch.build_report()
    finally:
        if old_force is None:
            os.environ.pop("V28_DUAL_PARENT_SHRINK_FRONTIER_FORCE_REPLAY", None)
        else:
            os.environ["V28_DUAL_PARENT_SHRINK_FRONTIER_FORCE_REPLAY"] = old_force

    unions = [compact_union(row) for row in report.get("unions") or [] if isinstance(row, dict)]
    return {
        "generated_at_utc": utc_now_iso(),
        "source_probe": "probe_v28_dual_lane_parent_shrink_frontier_watch.build_report(force_replay=1)",
        ...
        "weights": (report.get("state") or {}).get("weights"),
        "unions": unions,
...
def write_report(report: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "|---|---:|---|---:|---:|---:|---:|---:|---:|---|",
            (
                f"| `{best.get('frontier_label')}` | {best.get('frontier_weight')} | `{best.get('policy')}` | "
                f"{best.get('settled')} | {best.get('wins')}/{best.get('losses')} | "
    ...
    for row in report.get("unions") or []:
        if not isinstance(row, dict):
            continue
        lines.append(
            f"| `{row.get('frontier_label')}` | {row.get('frontier_weight')} | `{row.get('policy')}` | "
            ...
            f"{money(row.get('sidecar_add_net_cents'))} | {', '.join(str(item) for item in row.get('blockers') or [])} |"
        )
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
