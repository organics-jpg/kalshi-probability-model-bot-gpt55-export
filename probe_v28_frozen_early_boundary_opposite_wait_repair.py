"""Frozen forward validator for early-boundary opposite-side wait/repair.
...
Frozen rule:
    ...
    immediately. Wait for a same-market opposite-side row aged to <=480
    seconds-to-close with p>=0.50 and nonnegative executable edge, allowing up
    to 480 seconds of delay. If none appears, abstain. Restore the 75% coverage
    floor with clean calmer repair rows.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_early_boundary_opposite_wait_repair_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_early_boundary_opposite_wait_repair_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_early_boundary_opposite_wait_repair_latest.md"
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "base_policy": POLICY,
        "candidate": "early_boundary_wait480_p50_opposite_side_delay480",
        "danger_rule": "seconds_to_close>=780 and abs_d_sigma<=0.45 and recross_hazard_score>=0.55",
        "wait_rule": "first same-market opposite-side row with seconds_to_close<=480, p>=0.50, executable raw edge>=0, max delay 480s",
        ...
        "physics": (
            "Early near-boundary high-recross confidence is often a fragile first thesis. "
            "If clock decay flips the executable edge to the opposite side, the old thesis was path noise; "
            "if it does not, the market is skipped and broad coverage is repaired from calmer geometry."
        ),
        "params": {
            "danger_mode": "early_boundary",
            "wait_stc": 480.0,
            "p_floor": 0.50,
            "side_mode": "opposite_side",
            "max_delay_seconds": 480.0,
        },
    }
...
def build_report() -> dict[str, Any]:
    ...
    replacement_summary = candidate.get("replacement_summary") or {}
    return {
        "diagnostic": "frozen_early_boundary_opposite_wait_repair",
        "freeze": state,
        ...
        "interpretation": [
            ...
            f"Same-market opposite replacements: {replacement_summary.get('entries')}; replacement net {replacement_summary.get('net_cents')}c.",
...
def write_md(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# v28 Frozen Early-Boundary Opposite Wait Repair",
        "",
    ...
    for label, key in [
        ...
        ("opposite_waits", "replacement_summary"),
