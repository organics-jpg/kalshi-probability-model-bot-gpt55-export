"""Frozen forward watch for feature-gate entry plus guarded exit variants.

Research-only; no live bot changes or orders.

This tests the second mix/match idea from the top-candidate audit: the
boundary-clock observable feature gate as an entry filter, with book-gap and
loss-guarded exit handling applied only where a selected row can be matched to
a real live v28 exit row. Reconstructed/rejected rows remain entry-signal
evidence only; they do not get synthetic exit benefits.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_feature_gate_book_gap_exit_stack_state.json"
EXIT_SOURCES = {
...
OUT_JSON = OUT_DIR / "v28_frozen_feature_gate_book_gap_exit_stack_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_feature_gate_book_gap_exit_stack_latest.md"
...
def load_or_create_state() -> dict[str, Any]:
    ...
    if STATE_JSON.exists():
        ...
        if state.get("freeze_ts_utc"):
            ...
            physics = (
                "Entry gate avoids weak boundary/recross rows; guarded exits avoid selling "
                "into spread/turbulence when the hold thesis remains alive while rejecting "
                "holds with bad book/fair-drawdown structure."
            )
            if state.get("exit_rule") != exit_rule:
                state["exit_rule"] = exit_rule
                changed = True
            if state.get("physics") != physics:
    ...
    state = {
        ...
        "candidate_family": "feature_gate_book_gap_exit_stack",
        "entry_rules": RULES,
        "exit_rule": "Use frozen book-gap/loss-guard exit candidates only for unambiguous matched live v28 exit rows.",
        "physics": (
            "Entry gate avoids weak boundary/recross rows; guarded exits avoid selling "
...
def build_report() -> dict[str, Any]:
    ...
    surfaces = {
        "post_stack_entry": entry_surfaces(freeze_ts),
        "post_stack_bridge": bridge_surfaces(freeze_ts),
    }
    lanes = []
    ...
    for exit_source, path in EXIT_SOURCES.items():
        ...
        lanes.extend(
            [
                evaluate_lane("post_stack_entry", exit_source, freeze_ts, surfaces["post_stack_entry"], exits),
                evaluate_lane("post_stack_bridge", exit_source, freeze_ts, surfaces["post_stack_bridge"], exits),
            ]
        )
    return {
