"""Forward watch for loss-control refinement of reduce-exit suppression.

Research-only; no live bot changes or orders.

The blanket p_hold>=0.75 probability-reduce suppression recovers clipped
winners but also suppresses a small number of true losers. This freezes several
observable refinements from a new timestamp so future rows can test whether
loss-control cost can be reduced without giving back the winner recovery.
"""
...
STATE_JSON = OUT_DIR / "v28_frozen_exit_reduce_loss_control_refinement_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_exit_reduce_loss_control_refinement_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_exit_reduce_loss_control_refinement_latest.md"

MIN_SETTLED = 30
MIN_FULL_LOSS_CUSHION = 3

RULES = {
    "reduce_suppress_p_hold_ge_079": {
        "p_hold_min": 0.79,
        "drawdown_max": None,
        "drawdown_lte_zero": False,
        "physics": "Only suppress reduce exits when held-side probability is very high.",
    },
    "reduce_suppress_p_hold_ge_075_drawdown_lte_2p5": {
        "p_hold_min": 0.75,
        "drawdown_max": 2.5,
        "drawdown_lte_zero": False,
        "physics": "Suppress reduce exits only when the fair-value drawdown is small.",
    },
    "reduce_suppress_p_hold_ge_079_or_drawdown_lte_2p5": {
        "p_hold_min": 0.75,
        "drawdown_max": None,
        "drawdown_lte_zero": False,
        "or_p_hold_min": 0.79,
        "or_drawdown_max": 2.5,
        "physics": "Permit suppression if either held probability is very high or fair-value drawdown is small.",
    },
    "reduce_suppress_p_hold_ge_075_fair_drawdown_lte_0": {
        "p_hold_min": 0.75,
        "drawdown_max": None,
        "drawdown_lte_zero": True,
        "physics": "Suppress reduce exits only when fair value still favors holding over the exit mark.",
    },
}
...
def load_or_create_state() -> dict[str, Any]:
    ...
    state = {
        ...
        "candidate_family": "exit_reduce_loss_control_refinement",
        "origin": "Derived from the reduce-suppression risk ledger; earlier rows are diagnostic only.",
        "rules": RULES,
        "strict_forward_note": "Only post_refinement_birth lanes count as forward evidence.",
    }
...
def should_suppress(row: dict[str, Any], rule: dict[str, Any]) -> bool:
    ...
    if p_hold is None or drawdown is None:
        return False
    or_p_hold = rule.get("or_p_hold_min")
    or_drawdown = rule.get("or_drawdown_max")
    if or_p_hold is not None and or_drawdown is not None:
        return p_hold >= float(or_p_hold) or drawdown <= float(or_drawdown)
    if p_hold < float(rule.get("p_hold_min") or 0.0):
        return False
    if rule.get("drawdown_lte_zero") and drawdown > 0.0:
        return False
    drawdown_max = rule.get("drawdown_max")
    if drawdown_max is not None and drawdown > float(drawdown_max):
        return False
    return True
...
def build_report() -> dict[str, Any]:
    ...
    lanes.append(evaluate_lane("post_refinement_birth", str(state["freeze_ts_utc"])))
    report = {
...
def write_md(report: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Refinement freeze UTC: `{(report.get('state') or {}).get('freeze_ts_utc')}`",
        "",
        "## Interpretation",
        "",
    ]
