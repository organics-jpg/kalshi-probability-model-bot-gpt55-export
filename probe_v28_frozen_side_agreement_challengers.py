"""Frozen forward gate for side-agreement meta candidates."""
...
from probe_v28_side_agreement_meta_candidate import build_report as build_meta_report


ROOT = Path(__file__).resolve().parent
OUT_DIR = ROOT / "logs" / "edge_research"
STATE_JSON = OUT_DIR / "v28_frozen_side_agreement_challengers_state.json"
OUT_JSON = OUT_DIR / "v28_frozen_side_agreement_challengers_latest.json"
OUT_MD = OUT_DIR / "v28_frozen_side_agreement_challengers_latest.md"

CHALLENGERS = [
    {
        "policy": "raw_when_same_else_first_side_raw_later_book_p60_edge0",
        "role": "side_agreement_first_side_wait",
        "physics": "Use raw broad timing when the p60 forgetting candidate agrees with raw side; if side flips, wait for the p60 forgetting/book-anchored row.",
    },
    {
        "policy": "raw_when_same_else_rmt_repetition_forget_p60_edge0",
        "role": "side_agreement_rmt_wait",
        "physics": "Use raw broad timing when the RMT repetition candidate agrees with raw side; if side flips, wait for the RMT p60 row.",
    },
]
...
def normalize_meta_policy(row: dict[str, Any]) -> str:
...
def build_report() -> dict[str, Any]:
    ...
    for item in state.get("policies") or CHALLENGERS:
        policy = item["policy"]
        policy_rows = [{**row, "policy": policy} for row in future_rows if normalize_meta_policy(row) == policy]
